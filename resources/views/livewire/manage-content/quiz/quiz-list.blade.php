<div class="flex min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
    ...
    <div class="flex-1 p-6 overflow-auto">
        <div class="max-w-7xl mx-auto">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($quizzes as $quiz)
                                <tr class="hover:bg-gray-50/50 transition duration-150 ease-in-out">
                                    ...
                                    <td class="px-6 py-5">
                                        <ul class="space-y-2">
                                            @if(is_array($quiz->answers) || is_object($quiz->answers))
                                                @if(count($quiz->answers) > 0)
                                                    @foreach($quiz->answers as $answer)
                                                        <li class="flex items-start">
                                                            <span
                                                                class="flex items-center justify-center h-5 w-5 bg-blue-100 text-blue-800 rounded-full mr-2 mt-0.5 flex-shrink-0">
                                                            ...
                                                            <span class="text-sm text-gray-700">{{ $answer['text'] }} (Score:
                                                                {{ $answer['score'] ?? 'N/A' }})</span>
                                                        </li>
                                                    @endforeach
                                                @else
                                    ...
                                    <td colspan="3" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center justify-center space-y-4">
                                            <div class="p-4 bg-indigo-50 rounded-full">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-indigo-400"
                                            ...
                                            <p class="text-gray-500 max-w-md text-center">Create your first skin assessment
                                                question to help users understand their skin type and concerns.</p>
                                            <button wire:click="showAddForm"
                                                class="mt-2 inline-flex items-center px-5 py-2.5 border border-transparent text-sm font-medium rounded-full shadow-sm text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 transition duration-200">
            ...
            @if($quizzes->hasPages())
                <div
                    class="mt-8 px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6 bg-white rounded-b-xl shadow-sm">
