<div class="fixed inset-0 bg-gray-900/70 backdrop-blur-sm flex items-center justify-center p-4 z-50 transition-opacity duration-300 ease-in-out"
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto transform transition-all duration-300 ease-out">
        <!-- Modal Header -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <h3 class="text-sm font-semibold text-gray-900 mb-2">Considerations</h3>
                            <ul class="space-y-2">
                                @if(is_array($productDetails['negative']) && count($productDetails['negative']) > 0)
                                    @foreach($productDetails['negative'] as $negativeReview)
                                        <li class="flex items-start">
                                            <svg class="h-5 w-5 text-amber-500 mr-2 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    @endforeach
                                @else
                                    <p class="text-gray-500 italic">No considerations noted</p>
                                @endif
                            </ul>
                        </div>
                    </div>
